...
Everytime you finish a 25min Pomodoro, you get coin rewarded"""),
    ...
    def set_time(self,hour,minute):
        ...
        try:
            ...
            self._set_timePicker(hour_int, minute_int)
        except ValueError:
            # 处理转换失败的情况
            print(f"无效的时间参数: hour={hour}, minute={minute}")
    ...
    def _set_timePicker(self, hour, minute):
        print(f"设置时间为{hour}时{minute}分")

        # 检查时间值是否在有效范围内
        if not (0 <= hour <= 23 and 0 <= minute <= 59):
            print(f"时间值超出范围: hour={hour}, minute={minute}")
            return

        # 检查timePicker是否存在
        if not hasattr(self, 'timePicker') or self.timePicker is None:
            print("timePicker未正确初始化")
            return

        try:
            time_adjusted = QTime(hour, minute)
            self.timePicker.setTime(time_adjusted)
            # 强制更新界面
            self.timePicker.update()

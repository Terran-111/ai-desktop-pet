...
class DashboardMainWindow(FluentWindow):
    ...
    def _handle_set_focus_time(self, hour, minute):
        try:
            ...
            if hasattr(self.window.taskInterface, 'focus_panel'):
                self.window.taskInterface.focus_panel.set_time(hour, minute)
        except Exception as e:
            print(f"处理设置时间时出错: {e}")
    def closeEvent(self, event):
        event.ignore()  # 忽略那个接近的事件
        self.hide()

    def switch_to_task_interface(self):
        """自动切换到任务界面"""
